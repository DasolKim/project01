...
public class AuthController {
  ...
  public Object loginInfo(HttpSession session) {
    ...
    if(member == null) {  // 로그인 안한 경우
      ...
      result.put("state", "yes"); // 로그인 한 경우
      result.put("data", member); // 그 로그인한 정보를 달라.
    }
    return result;
  }
  
  @RequestMapping("/setRefererUrl")
  public Object setRefererUrl(String refererUrl, HttpSession session) {
    Map<String, Object> result = new HashMap<String, Object>();
    session.setAttribute("refererUrl", refererUrl);
    result.put("data", "yes");  //여기서 응답("yes")은 상실여부는 필요없음.
